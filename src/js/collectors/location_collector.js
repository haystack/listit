...
L.make.models.LocationMonitor = Backbone.Model.extend({
    initialize: function() {
        ...
        // Feature Recognizer
        L.vent.on("note:request:features", function(note, features) {
    ...
    getDistance : function(p1, p2) {
        ...
        var halfSinDelLon = Math.sin(this.toRadians(p1.longitude - p2.longitude)/2);
        var cosLat1 = Math.cos(this.toRadians(p1.latitude));
        var cosLat2 = Math.cos(this.toRadians(p2.latitude));
        var a = halfSinLat*halfSinLat + halfSinLon*halfSinLon*cosLat1*cosLat2;
        return 2*this.radius*Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
    },
    isNear : function(p1, p2) {
        return -.5 < this.getDistance(p1, p2) < .5;
    },
