...
Backbone.View = Backbone.View.extend({
  delegateEvents : function(events) {
    ...
    for (var key in events) {
      ...
      if (selector === '') {
        if (keyCombination) {
          this.$el.on(eventName, null, keyCombination, method);
        } else {
          this.$el.on(actualEventName, method);
        }
      } else {
        if (keyCombination) {
          this.$el.on(eventName, selector, keyCombination, method);
        } else {
          this.$el.on(actualEventName, selector, method);
        }
      }
    }
  }
