...
L.make.notes.StoredNoteCollection = L.make.notes.BaseNoteCollection.extend({
    ...
    update: function(notes) {
        ...
        _.each(notes, function(note) { 
            ...
            if (!myNote) throw new Error("Can't update non-existant note.", note);
            if (!myNote.set(note)) throw new Error("Note failed to validate", myNote, note);
            myNote.save();
        });
    }
});
...
L.make.notes.FilterableNoteCollection = L.make.notes.BaseNoteCollection.extend({
    ...
    */
    ...
    next: function(num) {
        ...
        var that = this;
        var start, end;
        num || (num = this._offset);

        if (this._lastSearchedNoteId) {
            // Return if there are no notes to search.
            var last_note = L.notes.last();
            if (!last_note) return;

            // Return if at end.
            if (this._lastSearchedNoteId === last_note.id) return;
        ...
        var last_note = L.notes.at(end);
        if (last_note) {
            this._lastSearchedNoteId = last_note.id;
